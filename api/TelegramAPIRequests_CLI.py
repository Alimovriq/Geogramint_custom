...
def geolocate_AllEntities_Nearby(
        api_id, api_hash, latitude, longitude, pictures):
    ...
    try:
        ...
        typer.echo(
            typer.style(
                "[WARNING] cache_telegram folder already exist",
    ...
    with TelegramClient(
            ...
            lang_code="en", system_lang_code="ru-RU") as client:
        client.connect()
        ...
        ListofGroup = ressources_cli.generate_ListOfGroups(
            channelsList, peersList)
        ListofUser = ressources_cli.generate_ListOfUsers(usersList, peersList)
        if pictures:
            with Progress() as progress:
                task1 = progress.add_task(
                    "[yellow] Searching Users...", total=len(ListofUser))
                # create cache file for users profiles pictures
                try:
                    os.mkdir("cache_telegram/users")
                except OSError:
                    typer.echo(typer.style(
                        "[WARNING] cache_telegram/users already exist",
                        fg=typer.colors.YELLOW, bold=True))
                ...
                try:
                    ...
                    typer.echo(typer.style(
                        "[WARNING] cache_telegram/groups already exist",
                ...
                while invalid_objects:
                    ...
                    if len(
                        ListofUser) > 0 and not ListofUser[
                            0].id.isnumeric():
                        ListofUser.pop(0)
                        invalid_objects = True
                    if len(
                        ListofGroup) > 0 and not ListofGroup[
                            0].id.isnumeric():
                        ListofGroup.pop(0)
                        invalid_objects = True
                ...
                for elm in ListofUser:
                    ...
                    client.download_profile_photo(
                        int(elm.id),
                        "cache_telegram/users/" + elm.id)

                # download of groups profile pics
                task2 = progress.add_task(
                    "[grey] Searching Groups...",
                    total=len(ListofGroup))
                for elm in ListofGroup:
                    progress.update(task2, advance=1)
                    if not elm.id.isnumeric():
                        continue
                    client.download_profile_photo(
                        int(elm.id), "cache_telegram/groups/" + elm.id)
