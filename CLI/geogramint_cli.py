...
from api.TelegramAPIRequests_CLI import geolocate_AllEntities_Nearby
from CLI import ressources_cli, settings_cli, surveillance_cli
from utils.mongo_db_operations import Database_operations
...
def version(
        version: bool = typer.Option(
            False, "--version",
            help="Show the version", callback=version_callback
        )
):
    pass
...
def set_hash(hash: str):
    ...
    typer.echo(typer.style(
        "API_Hash Updated !", fg=typer.colors.GREEN, bold=True))
...
def set_id(id: int):
    ...
    typer.echo(
        typer.style("API_ID Updated !", fg=typer.colors.GREEN, bold=True))
...
def set_phone(phone: str):
    ...
    typer.echo(
        typer.style(
            "Phone Number Updated !", fg=typer.colors.GREEN, bold=True))


@CLI.command(rich_help_panel='Config Commands')
def set_report_settings(
    extended_report: bool = typer.Option(
        False, help="enable or disable extended report")):
    ...
    if extended_report:
        typer.echo(typer.style(
            "Extended Report Set !", fg=typer.colors.GREEN, bold=True))
    else:
        typer.echo(
            typer.style(
                "Extended Report Unset !", fg=typer.colors.GREEN, bold=True))
...
def set_config(
    ...
    typer.echo(typer.style(
        "Config Updated !", fg=typer.colors.GREEN, bold=True))
...
def start_scan(
    lat: float,
    lon: float,
    output_json: str = typer.Option("cache_telegram",
                                    help="Directory Path"),
    ...
    profile_pictures: bool = typer.Option(
        True, help="enable or disable profile pictures download")):
    print(logo_ascii)
    api_id, api_hash, phone_number, extended_report = settings_cli.loadConfig()
    typer.echo(
        typer.style(
            "Config Loaded !", fg=typer.colors.GREEN, bold=True))

    users, groups, dt_string = geolocate_AllEntities_Nearby(
        api_id, api_hash, lat, lon, profile_pictures)

    if output_json[-1] == '/':
        output_json = output_json[:-1]
    json_string_user = json.dumps(
        [ob.__dict__() for ob in users], ensure_ascii=False)
    with codecs.open(output_json + '/users.json', 'w', 'utf-8') as f:
        f.write(json_string_user)
    json_string_group = json.dumps(
        [ob.__dict__() for ob in groups], ensure_ascii=False)
    ...
    table = Table(
        "ID", "First Name", "Last Name", "Username", "Phone", "Distance")
    for elm in users:
        if elm.distance == '500':
            color = '[bright_green]'
        ...
        if elm.phone is not None:
            ...
            phone = ""
        table.add_row(
            color + elm.id, color + elm.firstname[1:-1], lastname, username,
    ...
    for elm in groups:
        ...
        table_g.add_row(
            color + elm.id, color + elm.name[1:-1], color + elm.distance)
    console.print(table_g)

    if output_pdf != "":
        if output_pdf[-1] == '/':
            output_pdf = output_pdf[:-1]
        ressources_cli.generate_pdf_report(
            users, groups, lat, lon, dt_string, output_pdf, extended_report)

    if output_osintracker != "":
        if output_osintracker[-1] == '/':
            output_osintracker = output_osintracker[:-1]
        ressources_cli.generate_osintracker_investigation(
            users, groups, lat, lon, output_osintracker, extended_report)

    # Добавление объектов пользователей и групп в БД MongoDB.
    DB_MONGO.insert_collection(DB_MONGO.user_collection, users, dt_string)
    DB_MONGO.insert_collection(DB_MONGO.group_collection, groups, dt_string)
...
def reset_scan():
    ...
    typer.echo(
        typer.style(
            "cache_telegram deleted", fg=typer.colors.RED, bold=True))

    # Удаление всех объектов БД MongoDB.
    DB_MONGO.delete_db_elements()
    typer.echo(
        typer.style(
            "database elements deleted", fg=typer.colors.RED, bold=True))


@CLI.command(rich_help_panel='Actions Commands')
def surveillance(
    lat: float, lon: float, num_days: int = typer.Argument(
        help="Days of Active Surveillance"),
    ...
    while datetime.now() < end_date:
        ...
        users, groups, dt_string = geolocate_AllEntities_Nearby(
            api_id, api_hash, lat, lon, True)
        ressources_cli.generate_pdf_report(
            users, groups, lat, lon, dt_string, "cache_telegram", False)
        ...
        surveillance_cli.process_users(
            "cache_telegram/users.json", new_output_filename)

        with open(
            new_output_filename,
                "r", encoding="utf-8") as new_output_file:
        ...
        shutil.rmtree("cache", ignore_errors=True)
        # Sleep for an hour with random seconds to diminish the risks of
        # ban by Telegram
        time.sleep(3600 + random.randint(0, 60))

    shutil.rmtree("cache_telegram", ignore_errors=True)
    shutil.rmtree("cache", ignore_errors=True)


@CLI.command(rich_help_panel='Database Commands')
def print_db_elements(name_collection: str):
    """
    Вывод в консоль объектов БД по имени коллекции: Users, Groups.
    """

    DB_MONGO.print_db_elements(name_collection)
    typer.echo(typer.style(
        "database elements printed",
        fg=typer.colors.GREEN, bold=True))


@CLI.command(rich_help_panel='Database Commands')
def clear_db_elements():
    """
